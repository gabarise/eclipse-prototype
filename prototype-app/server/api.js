...
import { addSession, getTasks, getTask, getSlaveModules, getSlaveModule } from './data';
...
router.get('/slave', (req, res) => {
  getSlaveModules(req.query).then(function(slaves){ 
    return res.json(slaves);
  });
});

router.get('/slave/:id', (req, res) => {
  getSlaveModule(req.params.id).then((result) => {
    if (!result.slave) {
      res.status(404).end();
    } else {
      res.json(result);
